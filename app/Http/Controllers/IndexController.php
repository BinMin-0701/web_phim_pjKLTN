...
class IndexController extends Controller
{
  ...
  {
    $phimhot = Movie::where('phim_hot', 1)->where('status', 1)->orderBy('ngaycapnhat', 'DESC')->withCount('episode')->get();
  ...
  public function movie($slug)
  {
    ...
    // Đánh giá
    $rating = Rating::where('movie_id', $movie->id)->avg('rating'); //avg->tính tổng trung bình
    $rating = round($rating);
    ...
    return view('pages.movie', compact('rating', 'count_total','category', 'genre', 'country', 'movie', 'episode_tapdau','related', 'phimhot_sidebar','episode', 'episode_current_list_count'));
  }
  ...
  public function watch($slug, $tap)
  {
    ...
    $episode_current_list = Episode::with('movie')->where('movie_id', $movie->id)->get();
    $episode_current_list_count = $episode_current_list->count();
    return view('pages.watch', compact('category', 'genre', 'country', 'movie', 'related', 'phimhot_sidebar', 'episode', 'tapphim','server', 'episode_current_list_count'));

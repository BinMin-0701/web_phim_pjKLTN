        ...
        <div class="halim-item">
          <a class="halim-thumb" href="{{route('movie',$hot->slug)}}" title="{{$hot->title}}">
            ...
            <span class="status">
              ...
              @if($hot->phude==0)
              VietSub @if ($hot->thuocphim=='phimbo') - Tập {{$hot->episode_count}}/{{$hot->sotap}} @endif
              @else
              Thuyết Minh @if ($hot->thuocphim=='phimbo') - Tập {{$hot->episode_count}}/{{$hot->sotap}} @endif
              @endif
            </span>
          ...
          <div class="halim-item">
            <a class="halim-thumb" href="{{route('movie',$mov->slug)}}" title="{{$mov->title}}">
              ...
              <span class="episode"><i class="fa fa-play" aria-hidden="true"></i>
                @if($mov->phude==0)
                VietSub @if ($mov->thuocphim=='phimbo') - Tập {{$hot->episode_count}}/{{$mov->sotap}} @endif
                @else
                Thuyết Minh @if ($mov->thuocphim=='phimbo') - Tập {{$hot->episode_count}}/{{$mov->sotap}} @endif
                @endif
              </span>
              <div class="icon_overlay"></div>
              <div class="halim-post-title-box">
